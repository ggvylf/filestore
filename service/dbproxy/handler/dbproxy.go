...
import (
	...
	"github.com/ggvylf/filestore/service/dbproxy/mapper"
...
func (db *DBProxy) ExecuteAction(ctx context.Context, req *dbProxy.ReqExec, resp *dbProxy.RespExec) error {
	respList := make([]orm.ExecResult, len(req.Action))
	...
	for idx, singleAction := range req.Action {
		...
		respList[idx] = execRes[0].Interface().(orm.ExecResult)
	}

	// TODO: 处理异常
	resp.Data, _ = json.Marshal(respList)
	return nil
}
